...
func buildAgentPrompt(student model.TaskStudent, demand string, templates []string) string {
	...
	filledInfo := []string{
		...
		fmt.Sprintf("学校：%s", defaultString(student.SchoolName, "未填写")),
		fmt.Sprintf("学年：%s", defaultString(student.SchoolYear, "未填写")),
		fmt.Sprintf("班级：%s", defaultString(student.ClassName, "未填写")),
		fmt.Sprintf("是否来自推荐：%s", ternary(student.FromRecommend, "是", "否")),
...
func buildLocalAgentAdvice(student model.TaskStudent, demand string, templates []string, fallbackReason string) string {
	...
	lines := []string{
		...
		"",
		"## 一、总体判断",
		fmt.Sprintf("黑龙江 %s %s 口径下，你当前已填写的信息是：%s、%s、意向专业 %s。档案上下文为：学校 %s、学年 %s、班级 %s、推荐来源 %s。", defaultString(student.Subject, "未填写科类"), defaultString(yearOrAnalysisYear(student), "2025"), scoreText(student.Score), rankText(student.Rank), defaultString(student.TargetMajor, "未填写"), defaultString(student.SchoolName, "未填写"), defaultString(student.SchoolYear, "未填写"), defaultString(student.ClassName, "未填写"), ternary(student.FromRecommend, "是", "否")),
		summary,
		publicSummary,
		adjustSummary,
		"",
